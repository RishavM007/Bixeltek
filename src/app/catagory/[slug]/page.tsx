...
export default async function CategoryPage({ params }: { params: { slug: string } }) {
  const data: CategoryPageData = await client.request(GET_POSTS_BY_CATEGORY, {
    slug: params.slug,
  });

  const { categoryBy } = data;
  ...
  return (
    <div className="px-4 py-10 max-w-6xl mx-auto text-white">
      <h1 className="text-3xl font-bold mb-8 border-b border-neutral-700 pb-2">
        Category: {categoryBy.name}
      </h1>

      {categoryBy.posts.nodes.length === 0 ? (
        <p className="text-neutral-500">No posts found in this category.</p>
      ) : (
        <div className="grid gap-8 lg:grid-cols-2">
          {categoryBy.posts.nodes.map((post) => (
